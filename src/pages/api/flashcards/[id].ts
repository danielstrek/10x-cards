...
export const GET: APIRoute = async ({ params, locals }) => {
  try {
    ...
    if (isNaN(flashcardId)) {
      return new Response(JSON.stringify({ error: "Bad Request", message: "Invalid flashcard ID" }), {
        status: 400,
        headers: { "Content-Type": "application/json" },
      });
    }
    ...
    if (!flashcard) {
      return new Response(JSON.stringify({ error: "Not Found", message: "Flashcard not found" }), {
        status: 404,
        headers: { "Content-Type": "application/json" },
      });
    }
...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    const updatedFlashcard = await updateFlashcard(locals.supabase, flashcardId, dto, userId);
